...
public class RockEatCli {
	...
	public void execute(String[] args) {
		...
		options.addOption(new Option(URL, "url", true,
				"specifica l'indirizzo della pagina da analizzare"));
		options.addOption(new Option(DISABLE_TAGGING, "disable-tagging", false,
				"disabilita la scrittura automatica dei tag ID3"));
		options.addOption(new Option(EAT, "eat", false,
				"mangia tutte le tracce disponibili"));
		options.addOption(new Option(TESTING, "test", false,
				"verifica di essere in grado di scaricare"));
						...
						if (commandLine.hasOption(EAT)) {
							...
							for (Track track : album.getTracks()) {
								...
								String st = StringUtils.leftPad(">",
										progress + 1, "=")
										+ StringUtils.leftPad("", count
												- progress, " ");
								out.print("Download: ["
										+ st
										+ "] "
										+ Long.toString(controller
												.getDownloadedTracks() + 1)
							...
							if (controller.getDownloadedTracks() > 0) {
								...
								label = StringUtils
										.replace(
												...
												"{1}",
												FileUtils
														.byteCountToDisplaySize(controller
																.getBytesDownloaded()));
